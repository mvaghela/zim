...
include("function/monogramoption.php");
...
<?php include("include/header.php");
		...
		$sqlcpont="SELECT count(id) AS `countno` FROM `monogramoption` ";
		$count = $obj_db->select($sqlcpont);
		
		$sql="SELECT * FROM `monogramoption` ORDER BY `id` DESC LIMIT $s1,$s2";
				...
				<div class="block_head">
					...
					<div class="bheadr"></div>
							...
							<ul>
								...
								<!--<li>
									<a href="monogramoption.php?optionid=addnew">Add</a>
								</li>-->
							</ul>
				</div>
...
<?php 
		...
		<form action="" method="post" enctype="multipart/form-data" name="testform">
			<?php 
				$user_sql="SELECT * FROM `monogramoption` WHERE `id`='".$_REQUEST['optionid']."' ";
				$user_res=$obj_db->select($user_sql);
			?>
			<div style="padding:10px; margin-left:20px;" class="inner" >
				Name :
				<input class="text small" style="width:250px;" type="text" name="cname" readonly="readonly" value="<?php echo $user_res[0]['name'] ?>" />
				&nbsp;&nbsp; &nbsp; &nbsp;
				Price :
				<input class="text small" style="width:250px;" type="text" name="uname" value="<?php echo $user_res[0]['price']; ?>" />
<!--<input type="text" class="cp-input" value="rgb(123,42,87)"/>-->	
				...
				<a href="monogramoption.php" title="close">Close</a>
			</div>
		</form>
...
<?php }  ?>
                                  ...
                                  <table class="adminlist" cellpadding="3" cellspacing="1" width="100%">
                                        ...
                                        <tr class="row0">
										  ...
										  <td class="nrlbg<?php echo ($i & 1); ?>" nowrap="nowrap"><?php echo $result[$i]['price']; ?></td>
									
									  
	
	<td class="nrlbg<?php echo ($i & 1); ?>"  style="white-space:nowrap;"><a href="monogramoption.php?optionid=<?php echo $result[$i]['id']; ?>&amp;page=<?php if(isset( $_REQUEST['page'])){ echo $_REQUEST['page']; } else { echo 1; }?>"><img src="images/edit.png" /></a> &nbsp;&nbsp;&nbsp;<a href="monogramoption.php?del=<?php echo $result[$i]['id'] ?>&amp;page=<?php if(isset( $_REQUEST['page'])){ echo $_REQUEST['page']; } else { echo 1; }?>" OnClick="return confirm('Are you sure want delete this record?');"><img src="images/cross.png" /></a></td>
