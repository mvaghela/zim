...
?>
					<div class="filter-fabric-input-div">
						<select style="float:left;" class="styled categorycall" name="pattern" id="pattern">
							...
							?>
							<option <?php if($_SESSION['shirtfilter']['pattern']==$subcategory[$i]['optiontypeid']) { ?> selected="selected" <?php } ?> value="<?php echo $subcategory[$i]['optiontypeid']?>"><?php echo $subcategory[$i]['optiontypename']?></option>
							<?php } ?>
						</select>
					</div>
					<?php } ?>
...
?>
					<div class="filter-fabric-input-div">
						<select style="float:left;" class="styled categorycall" name="color" id="color">
							<option value="0">All <?php echo $subcategory[0]['optionname']?></option>
							<?php for($i=0;$i<count($subcategory);$i++) { ?>
							<option <?php if($_SESSION['shirtfilter']['color']==$subcategory[$i]['optiontypeid']) { ?> selected="selected" <?php } ?> value="<?php echo $subcategory[$i]['optiontypeid']?>"><?php echo $subcategory[$i]['optiontypename']?></option>
							<?php } ?>
						</select>
					</div>
					<?php } ?>
...
if($subcategory){ ?>
				<div class="filter-fabric-input-div">
						<select style="float:left;" class="styled categorycall" name="simplesort" id="simplesort" >
							<option <?php if($_SESSION['shirtfilter']['simplesort']=='productid') { ?> selected="selected" <?php } ?>value="productid">Standard</option>
							<option <?php if($_SESSION['shirtfilter']['simplesort']=='productname') { ?> selected="selected" <?php } ?> value="productname">Name</option>
							<option <?php if($_SESSION['shirtfilter']['simplesort']=='price') { ?> selected="selected" <?php } ?> value="price">Price</option>
						</select>
				</div>
<?php } ?>
...
if($subcategory){ ?>
				...
				<div class="filter-fabric-input-div">
						<select style="float:left;" class="styled categorycall" name="displayorder" id="displayorder" >
							<option <?php if($_SESSION['shirtfilter']['displayorder']=='asc') { ?> selected="selected" <?php } ?>value="asc">Ascending</option>
							<option <?php if($_SESSION['shirtfilter']['displayorder']=='desc') { ?> selected="selected" <?php } ?>value="desc">Desending</option>
						</select>
				</div>
<?php } ?>
			</div>
...
$.ajax({
        type: "GET",
        url: "shirt_refresh.php?page=<?php if($_GET['page']){ echo $_GET['page']; }else{echo '1';} ?>",
        data: dataString,
        cache: false,
        success: function(html){
			//alert(dataString);
