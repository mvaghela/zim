...
include("function/savecustomizepants.php");
?>
...
<?php if($_REQUEST['acn']=='savecust'){ ?>
			...
			<form action="savecustomizepants.php" method="post" >
			<table>
			<tr>
				<td colspan="3">
...
$category_sql="select * from `savepantcustomize` where `userid` = '".$_SESSION['userid']."' and `productid` = '".$_SESSION['product']."'";
...
?>
			...
			<tr>
				...
				<td> <select style="float:left; width:auto;" name="option" class="styled categorycall">
						...
						<option value="<?php echo $category[$i]['savepantcustomizeid']?>"><?php echo $category[$i]['customizename']?></option>
						<?php } ?>
					</select>
				</td>
			</tr>
		...
		<?php } else  { ?>
			...
			<tr>
				<td colspan="3">
					<div class="login_title">
						<h2>You have not saved any customization option for pant!!</h2>
				...
				<td colspan="3">
					<center>
						<div class="add-to-bag" style="float:none;">
								<a href="pantscustomize.php?productid=<?php echo $_REQUEST['productid'];?>" class="btn" >CUSTOMIZE</a>
						</div>
					</center>
				</td>
			</tr>
